...
export class Astar implements IAstar {
    ...
    /** A*辅助器 */
    private _astarHelper: IAstarHelper | null = null;
    /** 暂时缓存节点 */
    ...
    makePath(beginPosition: IAstarVec2, endPosition: IAstarVec2): Array<IAstarVec2> {
        ...
        let hValue = this._astarHelper.estimate(beginPosition, endPosition);
        if (hValue == 0) {
     ...
     * @returns
     */
    private inBoundary(position: IAstarVec2): boolean {
     ...
     * @returns
     */
    private positionToIndex(position: IAstarVec2): number {
     ...
     */
    private addToOpenList(index: number, position: IAstarVec2, hValue: number, parentNode: AstarNode | null = null): void {
     ...
     * @param endPosition 终点位置
     */
    private addAroundNodes(currentNode: AstarNode, endPosition: IAstarVec2): void {
        this.addIndexToCloseList(currentNode.index);

        this._astarHelper!.forEachAroundNodes((position: IAstarVec2) => {
            let newPos: IAstarVec2 = { x: currentNode.position.x + position.x, y: currentNode.position.y + position.y };
            if (this.inBoundary(newPos)) {
                ...
                if (!this._closeList.has(index) && !this._cachedOpenList.has(index)) {
                    ...
                    if (this._astarMap.check(newPos)) {
                        //地图测试通过，就加入开放列表
                        this.addToOpenList(index, newPos, this._astarHelper!.estimate(newPos, endPosition), currentNode);
                    } else {
     ...
     * @returns
     */
    private acquireAstarNode(index: number, position: IAstarVec2, hValue: number, parentNode: AstarNode | null = null): AstarNode {
